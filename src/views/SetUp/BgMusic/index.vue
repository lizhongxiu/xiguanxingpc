    ...
    <section class="main">
      <article class="main-breadcrumb">
        <el-breadcrumb separator="/" class="main-breadcrumb-wrap">
          ...
          <el-breadcrumb-item>
            <span class="main-breadcrumb-sp">背景音乐</span>
          </el-breadcrumb-item>
        </el-breadcrumb>
        <el-button
          v-allow="'060102'"
        ...
        <article class="main-table">
          ...
          >
            <template slot="name" slot-scope="scope">
              <span class="videoTitle" @click="videoPlay(scope.row.url)">{{
                scope.row.name
              }}</span>
            </template>
            <!--按钮-->
...
export default {
  ...
  methods: {
    _getList() {
      this.$api.operation.getMusicList(this.formData).then(res => {
        if (res.code == 200) {
    ...
    _openAddEdit(row, type) {
      ...
      obj["formType"] = type;
      if (type == "edit") {
        this.$api.operation.getMusic({ id: row.id }).then(res => {
          obj = {
            ...obj,
            ...res.data
          };
    ...
    _delete(index, row) {
      ...
      this.$confirm(`您是否确定删除`, "提示", {
        ...
        .then(() => {
          this.$api.operation.delMusic({ id: row.id }).then(res => {
            if (res.code == 200) {
              this.$message({
                type: "success",
                message: res.msg
