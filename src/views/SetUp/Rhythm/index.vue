  ...
  <div>
    <section class="main">
      ...
      <div class="main-content">
        <!--<article class="main-form" v-if="isvideoPage == false">-->
        <!--<el-form-->
        <!--ref="ruleForm"-->
        <!--:inline="true"-->
        <!--:model="formData"-->
...
export default {
  ...
  methods: {
    _getList() {
      this.$api.Rhythm.getBeatList(this.formData).then(res => {
        if (res.code == 200) {
    ...
    _openAddEdit(row, type) {
      ...
      obj["formType"] = type;
      if (type == "edit") {
        this.$api.Rhythm.getBeat({ id: row.id }).then(res => {
          obj = {
            ...obj,
            ...res.data
          };
    ...
    _delete(index, row) {
      ...
      this.$confirm(`您是否确定删除`, "提示", {
        ...
        .then(() => {
          this.$api.Rhythm.delBeat({ id: row.id }).then(res => {
            if (res.code == 200) {
              this.$message({
                type: "success",
                message: res.msg
